  ...
  <div class="wrap">
    ...
    <div class="footer-grid">
      ...
      <div class="footer-col footer-links-col">
        ...
        <!-- Local, not national: the one footer link pointing at a real
             place rather than a source or a feature. Small on purpose: this
             starts at one bar and stays footer-only until it's more than
             that. -->
        <a class="rss-link" href="cville.html">
...
/**
 * The one disclosure line for every post type: digests, previews, originals,
 * and Mondays alike. Used to vary by type (a digest calling itself fully
 * machine-written, a Monday post disclosing a facts/jokes split, an original
 * staying lighter since it's hand-written), but Ben wants one consistent
 * voice regardless of how much of a given post he actually dictated: the
 * take is his, the prose got an AI polish.
 */
...
/**
 * A one-off shoutout for a real person or business Ben wants to highlight
 * inside a post's own body. Deliberately not styled as a card (no border box,
 * no fill): a bordered/backgrounded box read as generic component-library
 * chrome once it was actually on the page, so this is a plain editorial
 * pull-quote instead: hairline rules top and bottom, the name itself carrying
 * the weight in large display type. `record.callout` is optional and
 * per-post (see data/mondays/2026-09-07.json for the first one, Högwaller
 * Brewing), not a general feature every post needs.
 */
...
/**
 * A hand-curated, growing list (config/local-spots.js), not a directory:
 * this starts at one entry and is meant to stay small and personally vetted
 * rather than turn into a business directory. Reuses partnerCallout() so a
 * spot mentioned inside a post and its listing here render identically.
 */
export function renderLocalSpotsPage({ siteName, siteUrl, sources, generatedAt, hasWeekly = false, isGameLive = false, spots = [] }) {
  const description = `Local Charlottesville spots worth knowing about if you're a Commanders fan in the area.`;
  const list = spots.length
    ? spots.map((s) => partnerCallout(s)).join('\n')
    : `<p class="page-intro">Nothing here yet. If you run a bar, restaurant, or shop in Charlottesville that wants to be the local Commanders hangout, <a href="contact.html">get in touch</a>.</p>`;
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<div class="hero">
${header('cville.html', hasWeekly, isGameLive)}
</div>

<main class="layout layout-wide">
  <div class="contact-page">
    <h1 class="podcasts-heading">Cville Spots</h1>
    <p class="page-intro">I live in Charlottesville. If this site ever becomes a place for local Commanders fans to find each other in person, this is where that starts: a short, hand-picked list of local spots that are Commanders-friendly. No algorithm, no paid placement, just places I'd actually send a friend.</p>
    ${list}
  </div>
</main>
